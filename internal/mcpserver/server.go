...
func (s *service) register(server *mcp.Server) {
	...
	s.registerResource(server, resourceSpec{
		...
		Description:    "Scheduled or unscheduled trip activities. Use one activity per actual stop, reservation, meal, tour, or experience. Activity creation requires latitude and longitude so the Tripsy map is populated. Use provider_reservation_code for the provider-issued reservation, confirmation, or booking code. MCP clients displaying activity dates/times must convert UTC starts_at/ends_at values into the activity timezone.",
		CreateAdvice:   "Set activity_type to the most specific built-in slug or visible custom category slug, include required latitude and longitude for the activity location, include address when known, use UTC starts_at/ends_at values with the local timezone for the activity location, store provider-issued reservation, confirmation, or booking codes in provider_reservation_code, and do not bundle multiple stops into one activity. MCP clients displaying activities must convert UTC starts_at/ends_at values into the activity timezone before formatting date/time. Custom category slugs are only valid on Activity objects through activity_type. MCP clients displaying activities must resolve non-built-in activity_type values through tripsy_categories_list so the correct custom activity category name, icon, and color are shown.",
		ExcludeData:    true,
		SkipCreate:     true,
	})
	addTool(server, toolName("tripsy", "activities", "create"), "Create Activity", "Create a Tripsy activity. Use one activity per actual stop, reservation, meal, tour, event, or experience. Activity creation requires latitude and longitude so the Tripsy map is populated; include address when known. Timed values are always UTC ISO-8601 timestamps; set timezone to the activity location's local IANA timezone. MCP clients displaying activities must convert UTC starts_at/ends_at values into the activity timezone before formatting date/time. Set activity_type to the most specific built-in slug or visible custom category slug, store provider-issued reservation, confirmation, or booking codes in provider_reservation_code, and do not invent ad hoc values such as sightseeing. Custom category slugs are only valid on Activity objects through activity_type. MCP clients displaying activities must resolve non-built-in activity_type values through tripsy_categories_list so the correct custom activity category name, icon, and color are shown. "+activityCategoryHint, additive(), s.activityCreate)
	s.registerResource(server, resourceSpec{
		Prefix:         "hostings",
		Title:          "Hosting",
		...
		Description:    "Hotel and lodging plans. Use provider_reservation_code for the provider-issued lodging reservation, confirmation, or booking code. MCP clients displaying lodging dates/times must convert UTC starts_at/ends_at values into the lodging timezone.",
		CreateAdvice:   "Use hostings for hotels and lodging rather than activities. Include name, address, latitude, longitude, UTC starts_at/ends_at values, the local timezone for the lodging location, and provider_reservation_code for provider-issued reservation, confirmation, or booking codes when known. MCP clients displaying lodging data must convert UTC starts_at/ends_at values into the lodging timezone before formatting local date/time.",
		ExcludeData:    true,
		SkipCreate:     true,
	})
	addTool(server, toolName("tripsy", "hostings", "create"), "Create Hosting", "Create a Tripsy hosting. Use hostings for hotels and lodging rather than activities. Timed values are always UTC ISO-8601 timestamps; set timezone to the lodging location's local IANA timezone. MCP clients displaying hostings must convert UTC starts_at/ends_at values into the lodging timezone before formatting local date/time. Include name, address, latitude, longitude, starts_at, ends_at, timezone, and provider_reservation_code for provider-issued reservation, confirmation, or booking codes when known.", additive(), s.hostingCreate)
	s.registerResource(server, resourceSpec{
		...
		Description:    "Flights, trains, cars, buses, ferries, walks, and other point-to-point travel. Use provider_reservation_code for provider-issued reservation, confirmation, or booking codes. MCP clients displaying transportation dates/times must convert UTC departure_at with departure_timezone and UTC arrival_at with arrival_timezone.",
		CreateAdvice:   "Use transportation_type for the segment kind and include departure/arrival coordinates when known. Use UTC departure_at/arrival_at values with local departure_timezone/arrival_timezone fields for display. Store provider-issued reservation, confirmation, or booking codes in provider_reservation_code; keep transport_number for the flight, train, bus, or service number. MCP clients must convert departure_at with departure_timezone and arrival_at with arrival_timezone before formatting local endpoint dates/times. For flights, use transportation_type airplane, set departure_description and arrival_description to airport IATA codes, include the airports' departure/arrival latitudes and longitudes, and omit name unless the user provided one. For transfer activities, use transportation_type roadtrip and include both departure and arrival names/descriptions, addresses, latitudes, and longitudes.",
		ExcludeData:    true,
		SkipCreate:     true,
	})
	addTool(server, toolName("tripsy", "transportations", "create"), "Create Transportation", "Create a Tripsy transportation for point-to-point movement. Timed values are always UTC ISO-8601 timestamps; set departure_timezone and arrival_timezone to each endpoint's local IANA timezone. MCP clients displaying transportations must convert departure_at with departure_timezone and arrival_at with arrival_timezone before formatting local endpoint dates/times. Use transportation_type for the segment kind and include departure/arrival names, addresses, latitudes, and longitudes when known. Store provider-issued reservation, confirmation, or booking codes in provider_reservation_code; use transport_number for the flight, train, bus, or service number. For flights, use transportation_type airplane, set departure_description and arrival_description to airport IATA codes, include the airports' departure/arrival latitudes and longitudes, and omit name unless the user provided one. For transfer activities, use transportation_type roadtrip and fill both departure and arrival locations completely. "+transportationCategoryHint, additive(), s.transportationCreate)
	s.registerResource(server, resourceSpec{
		Prefix:       "expenses",
		Title:        "Expense",
